    ...
    describe "Versioning based on #{input}" do
      ...
      results.keys.sort_by(&:to_s).each do |method|
        ...
        it "using #{method} #{input.inspect} becomes #{results[method].inspect}" do
          subject.build.release = "1"

          if method.to_s.include?("deb")
            subject.should_receive(:run_git_describe_internal).and_return(input)
            subject.build.packager = "puppetlabs"

          elsif method.to_s.include?("rpm")
            subject.should_receive(:run_git_describe_internal).and_return(input)

          else
            subject.stub(:uname_r) { "3.14159" }
            subject.stub(:is_git_repo) { true }
            subject.should_receive(:run_git_describe_internal).and_return(input)

          end

          subject.send(method).should == results[method]
        end
      end
    end
  end
